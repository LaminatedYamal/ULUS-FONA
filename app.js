...
window.toggleGame = function() {
    ...
    const game = document.getElementById('roulette-view');
    ...
    if (game.style.display === 'none') {
        ...
        toggleBtn.style.background = 'rgba(255, 215, 0, 0.1)';
...
function initRoulette() {
    ...
    const numberGrid = document.getElementById('number-bet-grid');
    
    if (wheel.children.length > 0) return; // Already init

    // Inject wheel segments
    ROULETTE_NUMBERS.forEach((num, i) => {
        const segment = document.createElement('div');
        segment.className = 'wheel-segment';
        segment.style.transform = `rotate(${(i * 360) / ROULETTE_NUMBERS.length}deg)`;
        
        let color = num === 0 ? '#016d29' : (isRed(num) ? '#e0080b' : '#1e1e1e');
        
        segment.innerHTML = `
            <div style="background: ${color}; width: 50px; height: 100px; clip-path: polygon(50% 0%, 0% 100%, 100% 100%); display: flex; justify-content: center; align-items: center; padding-top: 15px; font-size: 14px;">
                ${num}
            </div>
        `;
    ...
    for (let i = 1; i <= 36; i++) {
        ...
        btn.className = `bet-btn ${isRed(i) ? 'red' : 'black'}`;
        btn.textContent = i;
...
window.placeBet = function(type, value) {
    ...
    btns.forEach(btn => {
        if (type === 'color' && btn.classList.contains(value)) btn.classList.add('selected');
        if (type === 'type' && btn.textContent.toLowerCase() === value) btn.classList.add('selected');
        if (type === 'number' && parseInt(btn.textContent) === value) btn.classList.add('selected');
    });

    document.getElementById('current-bet-display').textContent = `${value.toString().toUpperCase()}`;
}

window.spinWheel = function() {
    if (isSpinning || !currentBet) {
        if (!currentBet) alert("Place a bet first!");
        return;
    }

    if (balance < 100) {
        alert("Insufficient GPT Balance!");
        return;
    }
    ...
    // Calculate rotation (Spin clockwise)
    const extraSpins = 5 + Math.floor(Math.random() * 5);
    // The rotation needs to align the resultIndex with the top (0 rotation is 0 index)
    // Each segment is 360/37 deg.
    const segmentDeg = 360 / ROULETTE_NUMBERS.length;
    const totalRotation = (extraSpins * 360) + (360 - (resultIndex * segmentDeg));
    
    wheel.style.transform = `rotate(${totalRotation}deg)`;
    
    document.getElementById('game-result').innerHTML = "Spinning...";
    ...
    setTimeout(() => {
        ...
        // Stabilize rotation for next spin
...
function checkWin(resultNum) {
    const resultColor = resultNum === 0 ? 'green' : (isRed(resultNum) ? 'red' : 'black');
    ...
    if (currentBet.type === 'color' && currentBet.value === resultColor) {
        won = true;
        multiplier = 2;
    } else if (currentBet.type === 'type') {
        if (currentBet.value === 'even' && isEven) won = true;
        if (currentBet.value === 'odd' && isOdd) won = true;
        multiplier = 2;
    } else if (currentBet.type === 'number' && currentBet.value === resultNum) {
        won = true;
        multiplier = 35;
    }
    ...
    if (won) {
        ...
        balance += winAmount;
        resultDiv.innerHTML = `<span class="win">WINNER! ${resultNum} (${resultColor.toUpperCase()}) - +${winAmount} GPT</span>`;
        resultDiv.className = "game-result win";
    } else {
        resultDiv.innerHTML = `<span class="loss">LOSS: ${resultNum} (${resultColor.toUpperCase()})</span>`;
...
function updateWallet() {
    document.getElementById('wallet-balance').textContent = `${balance.toLocaleString()} GPT`;
}
